# Example Approval Email: Portal Arena Studios Ltd — Game Backend

> **This is an example approval email.** It demonstrates what the applicant
> receives after their submission passes the review checklist and a
> certificate is issued.

---

**From:** sci-certification@greensoftware.foundation
**To:** jonas.berglund@portal-arena.example.com
**Subject:** SCI Self-Certification Approved — Portal Arena Studios Ltd — Game Backend — GSF-SCI-2026-00005

---

Dear Jonas,

Your SCI self-certification submission for **Portal Arena Game Backend v12.3** has been reviewed and **approved**.

**Certificate ID:** GSF-SCI-2026-00005
**Certificate:** https://badges.greensoftware.foundation/awards/sci/example-portal-arena-game-backend
**Public disclosure:** https://github.com/greensoftware-foundation/certification/blob/main/certifications/2026/GSF-SCI-2026-00005/disclosure.md
**Valid until:** 2027-06-18

Your submission has been published as your public disclosure (your contact email has been redacted). Badge assets (SVG, PNG, light and dark variants) are available from your certificate page.

Before using the badge, review the usage rules in the Applicant Guide — in particular, always include the "self-certified" qualifier when referencing ISO/IEC 21031:2024 conformity. The certification covers the Game Backend specifically; the player game client is out of scope and should not be implied to be certified.

To renew, submit an updated SCI measurement before **2027-05-19**. We will send a reminder 30 days before expiry.

If you have any questions, reply to this email.

Congratulations,

SCI Self-Certification Program
Green Software Foundation
sci-certification@greensoftware.foundation
